...
// 31.01.09  Rolläden nach oben um 7:20 (MO-FR)
// 22.03.09  Rolläden nach oben um 7:00 (MO-FR), 7:45 (SA), 8:00 (SO)
// 06.04.09  Abendlicht im April ab 19 Uhr, war 20Uhr
...
#define POWER_ON_LED_CYCLES   15 // 15 * 10ms blink takt
#define BEWAESSERN_CYCLES     60000
...
{
  BeweassernZeit.hours = 21;
  BeweassernZeit.minutes = 30;
...
{		  
  ...
  case stDarkness:    
	...
	roladen_demaerung=0;
	roller_shutters_delay = 90000; // 15 minuten
    break;  
  }
#ifdef DEBUG
   printf("%.2d:%.2d:%.2d - ", time.hours, time.minutes, time.seconds);
...
void InitVariables (void)
{
  ...
  aInput[SONNE1].SetTreshold(500, 500, 1200);
  

  aInput[SONNE2].SetChannel(6, 0);   
  aInput[SONNE2].SetTreshold(500, 500, 1200);
  
  aInput[TEMP].SetChannel(4, 0);     
  aInput[TEMP].SetTreshold(614, 600, 600);  // 22 Grad  Temp = AnalogWert/ 30.69 
  
  aInput[DEMMAERUNG].SetChannel(2, 0);     
  aInput[DEMMAERUNG].SetTreshold(400, 900, 900);
    
  aInput[REGEN].SetChannel(1, 1);     
  ...
  SensorCheckEnd.minutes = 15;
  SensorCheckEnd.seconds = 0;  
  
  WindowUpTime.hours = 7;
  WindowUpTime.minutes = 40;
  ...
  rollade[OST].SetAutomaticInterval(8, 30, 13, 0);	
  
  
  rollade[SUED].SetPin(ROLLADEN_SUED);
  rollade[SUED].SetAutomaticInterval(11, 0, 18, 0);	
  
  rollade[WEST].SetPin(ROLLADEN_WEST);
  rollade[WEST].SetAutomaticInterval(16, 0, 20, 0);	
  
  rollade[NORD].SetPin(ROLLADEN_NORD);
  rollade[NORD].SetAutomaticInterval(0, 0, 0, 0);	    
}
