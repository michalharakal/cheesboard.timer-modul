...
uint8_t watering_hours = 0;
uint8_t watering_minutes = 0; 
...
{   
  ...
  gd_hw_clear_scr();    
  Bar(0,56,127,64);
  ShowStr(2, 55, "Timermodul ver. 2009", -1);  
  ShowStr(2, 26, "Zeitempfang ... ", -1);    
  sprintf(auxstr, "Debug %d", dcf.debug);
  ShowStr(2, 16, auxstr, -1);    
  sprintf(auxstr, "Bits %d", dcf.bitcnt);
  ShowStr(2, 6, auxstr, -1);    
...
{   
   ...
   for (i = 0; i < 30; i++)
    ...
    ShowStr(1, 2, "Garten", -1);            
    ShowPoti(watering_hours, watering_minutes, 62, 2);
    gd_hw_update();	  
}
    ...
    // Ends up here if the last operation was a reception as Slave Address Match                  
          else
          {
		   ...
		   case CMD_SEND_DELAY_DATA:		     
		     ...
		     break;
		   case CMD_SEND_WATERING_DATA:		     	 
		     watering_hours = messageBuf[1];
		     watering_minutes = messageBuf[2]; 
			 break; 
			}
